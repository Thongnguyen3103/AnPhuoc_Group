...
OUT_XLSX     = r"\\hovmfs01\PKD\PhanPhoi\Thong.Nguyen\Report_BestSeller\04.Data_Xly\Data_BestSeller_Month.xlsx".replace("Phooi","PhanPhoi")
...
def main():
    ...
    data["Month"] = data["NgayChungTu"].dt.strftime('%m')
    ...
    # NOTE: bao gồm luôn 'Month' để nhóm riêng theo từng tháng
    group_cols = [
    ...
    # Áo: rank theo Report_item_name + Form + Color_group + Month
    mask_ao = data["Product_group"].astype(str).str.strip() == "Áo"
    if mask_ao.any():
        rank_cols_ao = ["Report_item_name", "Form", "Color_group", "Month"]
    ...
    # Quần, Váy, Vest bộ: rank theo Report_item_name + Form + Style + Month
    mask_other = data["Product_group"].astype(str).str.strip().isin(["Quần", "Váy", "Vest bộ"])
    if mask_other.any():
        rank_cols_other = ["Report_item_name", "Form", "Style", "Month"]
    ...
    # Những Product_group còn lại: group theo Report_item_name + Month
    mask_remaining = (data["rank"] == 0)
    if mask_remaining.any():
        remaining_data = data[mask_remaining].copy()
        remaining_data = remaining_data.sort_values(["Report_item_name", "Month", "SoLuong"], ascending=[True, True, False])
        remaining_data["rank"] = remaining_data.groupby(["Report_item_name", "Month"], dropna=False, sort=False).cumcount() + 1
