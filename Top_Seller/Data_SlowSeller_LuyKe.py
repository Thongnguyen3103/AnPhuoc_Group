...
OUT_XLSX     = r"\\hovmfs01\PKD\PhanPhoi\Thong.Nguyen\Report_BestSeller\04.Data_Xly\Data_SlowSeller_LuyKe.xlsx".replace("Phooi","PhanPhoi")
...
def build_dist_plan_exclude(base_date=None) -> list[str]:
    if base_date is None:
        base_date = pd.Timestamp.today().normalize()

    month_labels = []
    for offset in [1, 0, -1, -2]:
        target_date = base_date + pd.DateOffset(months=offset)
        month_labels.append(f"T{target_date.month:02d}.{target_date.year}")
...
def main():
    print("=" * 60)
    print("BẮT ĐẦU XỬ LÝ DỮ LIỆU BÁN CHẬM LŨY KẾ")
    print("=" * 60)
    ...
    print("\n⏰ Xử lý ngày tháng...")
    # Ngày chứng từ chỉ dùng để đọc dữ liệu; báo cáo này cộng lũy kế toàn bộ thời gian.
    data["NgayChungTu"] = pd.to_datetime(
    data["NgayChungTu"],
    errors="coerce",
    dayfirst=True
    )
    ...
    ts = pd.read_excel(PATH_DATABASE, sheet_name="E_T-Shirt", dtype=str)
    dm = normalize_columns(dm); kv = normalize_columns(kv); dt = normalize_columns(dt); ts = normalize_columns(ts)

    for c in ["Short_name","Brand","Group_Report","Report_item_name","Product_type","Gender","Product_group"]:
    ...
    # Kiểm tra & chuẩn bị E_T-Shirt nếu có
    ts_required = ["T-shirt_name"]  # Chỉ cần T-shirt_name, item_code tìm riêng
    ts_item_col = None
    # Tìm cột item_code
    for col in ts.columns:
        if keyize(col) == "itemcode":
            ts_item_col = col
            break
    
    ts_missing = [c for c in ts_required if c not in ts.columns]
    
    if not ts_missing and ts_item_col and len(ts) > 0:
        # Tạo mapping: item_code -> T-shirt_name
        ts_map = ts[[ts_item_col, "T-shirt_name"]].copy()
        ts_map = ts_map.rename(columns={ts_item_col: "item_code"})
        ts_map["item_code"] = ts_map["item_code"].astype(str).str.strip()
        ts_map["T-shirt_name"] = ts_map["T-shirt_name"].astype(str).str.strip()
        data["MaHang"] = data["MaHang"].astype(str).str.strip()
        print(f"✓ E_T-Shirt ready: {len(ts_map)} rows mapping")
    else:
        ts_map = None
    ...
    data = data.merge(
        dm[["Short_name","Brand","Group_Report","Report_item_name","Product_type","Gender","Product_group"]].rename(columns={"Short_name":"Short_name"}),
    ...
    dist_values_exclude = build_dist_plan_exclude()
    print(f"🔍 Lọc Dist_plan: chỉ giữ định dạng Txx.xxxx và bỏ {len(dist_values_exclude)} tháng gần nhất ({', '.join(dist_values_exclude)})...")
    before_distplan = len(data)
    # Chỉ lấy các thông tin có dạng Txx.xxxx
    mask_format = data["Dist_plan"].astype(str).str.strip().str.match(r"^T\d{2}\.\d{4}$", na=False)
    # Loại bỏ các tháng gần nhất
    mask_exclude = data["Dist_plan"].astype(str).str.strip().isin(dist_values_exclude)
    data = data[mask_format & ~mask_exclude]
    print(f"✓ Sau lọc Dist_plan: {len(data):,}/{before_distplan:,} dòng ({len(data)/before_distplan*100:.1f}%)")
    ...
    # Không tách theo tháng/quý: cộng lũy kế toàn bộ dữ liệu.
    group_cols = [
        "MaHang","Short_name","Code_Color",
        "Brand","Group_Report","Report_item_name","Product_type","Gender","Product_group",
        "Status","Operation","NoiDung",
    ...
    # Thay thế Report_item_name từ E_T-Shirt nếu có match (sau groupby)
    if ts_map is not None and len(ts_map) > 0:
        print("🔄 Thay thế Report_item_name từ E_T-Shirt nếu có match...")
        # Tạo mapping: item_code -> T-shirt_name
        mapping = dict(zip(ts_map["item_code"], ts_map["T-shirt_name"]))
        
        # Nếu data.MaHang match item_code, lấy T-shirt_name
        data["T_shirt_name_mapped"] = data["MaHang"].map(mapping).fillna("")
        matched = (data["T_shirt_name_mapped"] != "").sum()
        
        # Thay thế Report_item_name nếu có match
        data["Report_item_name"] = np.where(
            data["T_shirt_name_mapped"] != "",
            data["T_shirt_name_mapped"],
            data["Report_item_name"]
        )
        data = data.drop(columns=["T_shirt_name_mapped"])
        print(f"✓ Thay thế Report_item_name xong ({matched} rows matched)")

    print("🏆 Tính toán rank (BÁN CHẬM - xếp từ thấp đến cao)...")
    # Tạo cột rank
    data["rank"] = 0
    
    # Áo: rank lũy kế theo Report_item_name + Form + Color_group
    mask_ao = data["Product_group"].astype(str).str.strip() == "Áo"
    if mask_ao.any():
        rank_cols_ao = ["Report_item_name", "Form", "Color_group"]
        # Lấy dữ liệu áo, sort theo SoLuong TĂNG DẦN (bán chậm = số lượng thấp)
        ao_data = data[mask_ao].copy()
        ao_data = ao_data.sort_values(rank_cols_ao + ["SoLuong"], ascending=[True]*len(rank_cols_ao) + [True])
        ao_data["rank"] = ao_data.groupby(rank_cols_ao, dropna=False, sort=False).cumcount() + 1
        data.loc[mask_ao, "rank"] = ao_data["rank"]
    
    # Quần, Váy, Vest bộ: rank lũy kế theo Report_item_name + Form + Style
    mask_other = data["Product_group"].astype(str).str.strip().isin(["Quần", "Váy", "Vest bộ"])
    if mask_other.any():
        rank_cols_other = ["Report_item_name", "Form", "Style"]
        # Lấy dữ liệu, sort theo SoLuong TĂNG DẦN
        other_data = data[mask_other].copy()
        other_data = other_data.sort_values(rank_cols_other + ["SoLuong"], ascending=[True]*len(rank_cols_other) + [True])
    ...
    # Những Product_group còn lại: rank lũy kế theo Report_item_name
    mask_remaining = (data["rank"] == 0)
    if mask_remaining.any():
        remaining_data = data[mask_remaining].copy()
        remaining_data = remaining_data.sort_values(["Report_item_name", "SoLuong"], ascending=[True, True])
        remaining_data["rank"] = remaining_data.groupby(["Report_item_name"], dropna=False, sort=False).cumcount() + 1
    ...
    data_export = clean_for_excel(data_export)
    ...
    for attempt in range(max_retries):
        try:
            with pd.ExcelWriter(OUT_XLSX, engine="xlsxwriter", datetime_format="yyyy-mm-dd") as w:
                ...
                data_export.to_excel(w, sheet_name="Data", index=False)
            print("✓ Ghi file Excel xong")
            print(f"  - Ghi 1 sheet 'Data' với {len(data_export):,} dòng")
            break
    ...
    print("\n" + "=" * 60)
    print("✅ HOÀN THÀNH XỬ LÝ DỮ LIỆU BÁN CHẬM LŨY KẾ")
    print("=" * 60)
    print(f"📍 Địa chỉ file: {OUT_XLSX}")
    print(f"📊 Số dòng: {len(data_export):,} | Số cột: {data_export.shape[1]}")
    print("=" * 60)
