...
OUT_XLSX     = r"\\hovmfs01\PKD\PhanPhoi\Thong.Nguyen\Report_BestSeller\04.Data_Xly\Data_SlowSeller_Quarter.xlsx".replace("Phooi","PhanPhoi")
...
def build_dist_plan_exclude(base_date=None) -> list[str]:
    ...
    static_labels = ["HANG TANG", "HTT", "H.Đồng", "HANG THONG TIN", "Hàng SC"]
    return month_labels + static_labels
...
def main():
    print("=" * 60)
    print("BẮTĐẦU XỬ LÝ DỮ LIỆU BÁN CHẬM")
    print("=" * 60)
    ...
    print(f"🔍 Lọc bỏ Dist_plan ({', '.join(dist_values_exclude[:4])}, HANG TANG, HTT, H.Đồng, HANG THONG TIN, Hàng SC, Chưa PP)...")
    # Lọc bỏ các giá trị Dist_plan không mong muốn
    before_distplan = len(data)
    mask_distplan = data["Dist_plan"].notna() & (data["Dist_plan"].astype(str).str.strip() != "") & ~data["Dist_plan"].astype(str).str.strip().isin(dist_values_exclude)
    data = data[mask_distplan]
    ...
    # Fillna và group by (không cần Month nữa)
    ...
    # Áo: rank theo Quarter + Report_item_name + Form + Color_group
    mask_ao = data["Product_group"].astype(str).str.strip() == "Áo"
    if mask_ao.any():
        rank_cols_ao = ["Quarter", "Report_item_name", "Form", "Color_group"]
    ...
    # Quần, Váy, Vest bộ: rank theo Quarter + Report_item_name + Form + Style
    mask_other = data["Product_group"].astype(str).str.strip().isin(["Quần", "Váy", "Vest bộ"])
    if mask_other.any():
        rank_cols_other = ["Quarter", "Report_item_name", "Form", "Style"]
    ...
    # Những Product_group còn lại: group theo Quarter + Report_item_name
    mask_remaining = (data["rank"] == 0)
    if mask_remaining.any():
        remaining_data = data[mask_remaining].copy()
        remaining_data = remaining_data.sort_values(["Quarter", "Report_item_name", "SoLuong"], ascending=[True, True, True])
        remaining_data["rank"] = remaining_data.groupby(["Quarter", "Report_item_name"], dropna=False, sort=False).cumcount() + 1
    ...
    # Thứ tự cột khi xuất (BỎ Month)
    ordered_cols = [
        "MaHang","Short_name","Code_Color","NoiDung",
        "SoLuong","DonGia","Operation","Quarter",
        "Brand","Group_Report","Report_item_name","Product_type","Gender","Product_group",
        "Status",
        "Composition","Color_tone","Color_group","Form","Style","Attribute","Description","Price","Dist_plan","Production_month","Production_year",
    ...
    print("\n" + "=" * 60)
    print("✅ HOÀN THÀNH XỬ LÝ DỮ LIỆU BÁN CHẬM")
    print("=" * 60)
